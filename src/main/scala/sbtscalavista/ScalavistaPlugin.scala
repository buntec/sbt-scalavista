...
object GenerateConfig {
  def apply(sources: Seq[File], classpath: Seq[Attributed[File]], scalaBinaryVersion: String, scalacOptions: Seq[String]): Unit = {
    val json = JsObject(
      "classpath" -> classpath.map(_.data).mkString(":").toJson,
